...
def main():
    ...
    run_frequentist_optimization(m, pdb_h, avs, sr, f"frequentist_box{target_id}", max_cg_steps=150)
    ...
    ax_xy.scatter(aligned_cluster[:, 0], aligned_cluster[:, 1], c=norm_intensities, cmap='plasma', s=8 + 20*norm_intensities, alpha=0.8, zorder=2)
    ...
    ax_map.contourf(xx, yy, field, levels=25, cmap="viridis", alpha=0.9)
    ...
    fig.suptitle(f"Thesis Publication Figure: NPC Analysis (Box {target_id})", color='white', fontsize=22, fontweight='bold')
    
    out_path = os.path.join(_examples_dir, "figures", "methodology", f"thesis_final_result_box{target_id}.png")
